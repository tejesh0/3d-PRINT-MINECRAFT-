...
def man(position):
    x,y,z = position
    #print x
    with open('mant.stl', 'a') as fp:
            print "MAN"
            writer = ASCII_STL_Writer(fp)
            writer.add_faces(get_cube(x,y,z,0.5))
...
class Model(object):
    ...
    def hit_test(self, position, vector, max_distance=80):
    ...
    def _show_block(self, position, texture):
        ...
        with open('g.stl', 'wb') as fp:
            writer = ASCII_STL_Writer(fp)
            writer.add_faces(get_cube(x,y,z,5))
            #writer.close()
        print position
        man(position)
        print position
...
def main():
    window = Window(width=800, height=600, caption='EasyPrint', resizable=True)
